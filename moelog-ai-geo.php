  ...
  "moelog-ai-qna",
    ...
    add_settings_field(
      ...
      function () {
        ...
        echo '<p class="description">' .
          esc_html__("例:gpt-4o-mini 或 gemini-2.5-flash。", "moelog-ai-qna") .
          "</p>";
      },
      self::OPT_KEY,
      "general",
    ...
    add_settings_field(
      ...
      function () {
        ...
        $val =
          $o["system_prompt"] ??
          __("你是嚴謹的專業編輯,提供簡潔準確的答案。", "moelog-ai-qna");
        printf(
...
<p id="moelog-aiqna-help" style="color:#666; margin-top:6px;">
  ...
  private function get_questions_block($post_id)
  {
    ...
    foreach ($lines as $idx => $q) {
      ...
      $pretty = user_trailingslashit(home_url(self::PRETTY_BASE . "/" . $slug));
      $items .= sprintf(
  ...
  public function enqueue_styles()
  {
    $is_answer_page =
      ...
      (isset($_SERVER["REQUEST_URI"]) &&
        (preg_match("#/" . self::PRETTY_BASE . "/#", $_SERVER["REQUEST_URI"]) ||
  ...
  public function render_answer_page()
  {
    ...
    if (get_option("moelog_aiqna_geo_mode")) {
      // GEO 模式:只封鎖惡意爬蟲,允許 Googlebot/Bingbot
      $default_blocked = [
        "scrape",
        "curl",
        "wget",
        "Baiduspider", // 可選:中國搜尋引擎
        "SemrushBot", // SEO 工具爬蟲
        "AhrefsBot",
        "MJ12bot",
        "DotBot",
    ...
    // 套用 filter (GEO 模組的 allow_major_bots 可以進一步調整)
    ...
    if ($post_id && $v150_slug && $v150_hash) {
      ...
      foreach ($candidates as $idx => $qc) {
        ...
        if ($test_slug === $v150_slug . "-" . $v150_hash . "-" . $post_id) {
          $question = $qc;
    ...
    if (!$question) {
      ...
      if ($post_id && $question && $nonce && $ts && $sig) {
        ...
        $act =
          ...
          $post_id .
          "|" .
          substr(hash_hmac("sha256", (string) $post_id, $this->secret), 0, 12);
    ...
    if (!$post) {
      wp_die(__("找不到文章。", "moelog-ai-qna"), __("錯誤", "moelog-ai-qna"), [
        "response" => 404,
      ]);
    }
    ...
    if ($include) {
      ...
      if (function_exists("mb_strlen") && function_exists("mb_strcut")) {
        ...
        $context =
          strlen($raw) > $max_chars ? substr($raw, 0, $max_chars) : $raw;
      }
    }
  ...
  private function render_answer_html($post_id, $question, $answer)
  {
    ...
    if (get_option("moelog_aiqna_geo_mode")) {
      header(
        "Cache-Control: public, max-age=3600, s-maxage=" .
          self::CACHE_TTL .
...
// 站點預設（毫秒/字），不填就走程式內建 18ms
if (typeof window.MoelogAIQnA.typing_ms === 'undefined') {
  window.MoelogAIQnA.typing_ms = 18;
}
...
?>
<div class="moe-banner" <?php if ($banner_url): ?>style="background-image:url('<?php echo esc_url($banner_url); ?>')"<?php endif; ?> role="img" aria-label="<?php echo esc_attr($banner_alt); ?>"></div>
...
$safe_html = preg_replace_callback(
  ...
  function ($m) {
    $url = urldecode($m[1]);
    return '<span class="moe-url" title="' . esc_attr($url) . '">' . esc_html($url) . '</span>';
  },
  $safe_html
);
...
<div class="moe-original-section">
  ' . esc_html__( "原文連結", "moelog-ai-qna" ) . '<br>
...
?>
  ...
  <template id="moe-ans-source"><?php echo $safe_html . $original_html; ?></template>

  <noscript><?php echo $safe_html
    ? $safe_html
    : "<p>" . esc_html__("抱歉,目前無法取得 AI 回答,請稍後再試。", "moelog-ai-qna") . "</p>"; ?></noscript>
  <script nonce="<?php echo esc_attr($nonce); ?>">
  (function(){
    ...
    switch ($provider) {
      ...
      default:
        ...
        if (is_wp_error($resp)) {
          if (defined("WP_DEBUG") && WP_DEBUG) {
            error_log(
              "[Moelog AIQnA] HTTP Error: " . $resp->get_error_message(),
            );
          }
...
// 1) 啟動時/版本變更時 flush rewrite
register_activation_hook(__FILE__, function () {
    add_option('moe_aiqna_needs_flush', '1');
});
