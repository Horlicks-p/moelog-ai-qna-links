...
**不會將**訪客 IP、User-Agent 或其他訪客個人資料傳送給 AI provider。為防止公開端點濫用，站點本地會短期保存由 IP 與 WordPress site salt 產生、不可直接還原 IP 的匿名識別與限流計數。所有 AI provider 通訊均透過 HTTPS。
...
- 🐛 **修正:** 靜態快取在每次讀取時會呼叫 `touch()` 重置 mtime，導致快取永遠不會過期。已移除 `touch()` 呼叫，快取 TTL 現在可正常運作。
- 🐛 **修正:** 版本遷移函數 `moelog_aiqna_check_upgrade()` 的執行順序錯誤（1.8.3 先於 1.8.0），導致新安裝時 1.8.0 的 TTL 初始化遷移被跳過。
- 🐛 **修正:** `moelog_aiqna_clear_route_cache()` 實際上無法清除 PHP static 變數快取。改用 `$reset` 參數機制，現在可真正重置 `pretty_base` 和 `static_dir` 的快取值。
- ✨ **新功能:** 在「顯示設定」分頁新增「顯示問題區塊」開關（對應 `block_enabled` 設定，原本只能透過程式碼控制）。
- 🔧 **更新:** Gemini API 端點從實驗性的 `v1beta` 升級為穩定版 `v1`。
- 🔧 **修正:** 建立快取目錄時，`.htaccess` 保護檔案不再包含多餘的前導空白。
