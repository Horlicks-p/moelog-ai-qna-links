...
= 1.10.3 (2025-01-XX) – 後台UI重新設計 (Claude風格) =
- 🎨 **UI 重新設計:** 後台管理介面採用 Claude 風格設計，配色參考 sample_style.css。
- 🎨 **新增樣式:** 新增 `includes/assets/css/admin.css`，統一管理後台樣式。
- 🎨 **介面優化:** 主介面改為靠左對齊，移除置中佈局。
- 🎨 **間距調整:** 優化選單與內容區域之間的間距，視覺更緊密。
- 🎨 **圓角調整:** 主介面上方改為直角，與選單底部對齊，視覺更一致。
- 🎨 **按鈕佈局:** API Key 欄位的「顯示」和「測試連線」按鈕分開，一個靠左一個靠右。
- 🎨 **輸入框寬度:** 所有輸入框寬度從 100% 調整為 80%，版面更舒適。
- 🎨 **設計特點:** 簡潔現代、柔和圓角、清晰層次、舒適間距、柔和陰影、優雅過渡動畫。
