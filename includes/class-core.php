...
class Moelog_AIQnA_Core
{
    ...
    public function handle_save_post_pregenerate($post_id, $post)
    {
        ...
        $old_hash = get_post_meta($post_id, "_moelog_aiqna_content_hash", true);

        if ($old_hash === $new_hash) {
            if (defined("WP_DEBUG") && WP_DEBUG) {
                error_log(
                    "[Moelog AIQnA] SKIP pregenerate: unchanged. post={$post_id}"
                );
            }
        ...
        if (
            ...
            if (defined("WP_DEBUG") && WP_DEBUG) {
                error_log("[Moelog AIQnA] QUEUED pregenerate: post={$post_id}");
            }
        }
    }
    // =========================================
