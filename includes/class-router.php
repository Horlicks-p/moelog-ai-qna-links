...
class Moelog_AIQnA_Router
{
   ...
   * 解析當前請求並返回參數
   *
   * ✅ 優化: 先取得 post meta 並傳入子方法,避免重複查詢
   *
   * @return array|false {
  ...
  public function parse_request()
  {
    ...
    $query_post_id_legacy = isset($_GET["post_id"])
      ? intval($_GET["post_id"])
      : intval($query_post_id);

    // ✅ 優化: 確定 post_id 後,一次性取得所有 meta 資料
    $target_post_id = 0;
    if ($query_post_id && ($v150_slug || $token)) {
      $target_post_id = intval($query_post_id);
    } elseif ($query_post_id_legacy && $token) {
      $target_post_id = intval($query_post_id_legacy);
    }

    // ✅ 如果有 post_id,預先載入 meta 資料(只查詢一次)
    $questions_data = null;
    $langs_data = null;

    if ($target_post_id > 0) {
      $raw_questions = get_post_meta(
        $target_post_id,
        MOELOG_AIQNA_META_KEY,
        true,
      );
      $questions_data = $this->parse_questions_list($raw_questions);

      $langs_data = get_post_meta(
        $target_post_id,
        MOELOG_AIQNA_META_LANG_KEY,
        true,
      );
      $langs_data = is_array($langs_data) ? $langs_data : [];
    }
    ...
    // === 解析策略 2: 舊版 token 格式 ===
    if ($query_post_id_legacy && $token) {
      $result = $this->parse_legacy_token(
        $query_post_id_legacy,
        $token,
        $questions_data, // ✅ 傳入已查詢的資料
        $langs_data, // ✅ 傳入已查詢的資料
      );
   ...
   * 解析 v1.5.x 簡短 slug
   *
   * ✅ 優化: 接收預先查詢的 questions 和 langs 資料
   *
   * @param int        $post_id       文章 ID
   * @param string     $v150_slug     slug 前綴
   * @param string     $v150_hash     hash 部分
   * @param array|null $questions     預先查詢的問題列表
   * @param array|null $langs         預先查詢的語言設定
   * @return array|false
   */
  private function parse_v150_slug(
    ...
    foreach ($questions as $idx => $candidate_question) {
      $expected_slug = $this->slugify_question($candidate_question, $post_id);
   ...
   * @return array|false
   */
  private function parse_legacy_token(
    $post_id,
    $token,
    $questions = null,
    $langs = null,
  ...
  {
    $raw = hash_hmac("sha256", $post_id . "|" . $question, $this->secret, true);
