...
class Moelog_AIQnA_Cache
{
    ...
    public static function save($post_id, $question, $html)
    {
        ...
        if ($result === false) {
            if (defined("WP_DEBUG") && WP_DEBUG) {
                error_log(
                    sprintf(
                        "[Moelog AIQnA Cache] Failed to save static file: %s",
                        $path
                    )
                );
            }
    ...
    public static function load($post_id, $question)
    {
        ...
        if ($html === false) {
            if (defined("WP_DEBUG") && WP_DEBUG) {
                error_log(
                    sprintf(
                        "[Moelog AIQnA Cache] Failed to load static file: %s",
                        $path
                    )
                );
            }
    ...
    public static function clear_expired()
    {
        ...
        if ($count > 0 && defined("WP_DEBUG") && WP_DEBUG) {
            error_log(
                sprintf(
                    "[Moelog AIQnA Cache] Cleared %d expired static files",
                    $count
                )
            );
        }
    ...
    public static function clear_all_transients()
    {
        ...
        if (defined("WP_DEBUG") && WP_DEBUG) {
            error_log(
                sprintf("[Moelog AIQnA Cache] Cleared %d transients", $count)
            );
        }
