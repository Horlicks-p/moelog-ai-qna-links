  ...
  public function render_model_field()
{
    ...
    <input type="text"
           ...
           data-default-openai="<?php echo esc_attr(Moelog_AIQnA_AI_Client::DEFAULT_MODEL_OPENAI); ?>"
           data-default-gemini="<?php echo esc_attr(Moelog_AIQnA_AI_Client::DEFAULT_MODEL_GEMINI); ?>"
           data-default-anthropic="<?php echo esc_attr(Moelog_AIQnA_AI_Client::DEFAULT_MODEL_ANTHROPIC); ?>">
    ...
    <p class="description" id="model-hint-openai" style="display:none;">
        <?php esc_html_e('OpenAI 模型範例: gpt-4o-mini, gpt-4o, gpt-4.1, gpt-4.1-mini', 'moelog-ai-qna'); ?>
    </p>
    ...
    <p class="description" id="model-hint-anthropic" style="display:none;">
        <?php esc_html_e('Anthropic 模型範例: claude-sonnet-4-5-20250929, claude-opus-4-1', 'moelog-ai-qna'); ?>
    </p>
...
public function sanitize_settings($input)
{
    ...
    // =========================================
    $default_heading = __('還有其他問題嗎?以下是 AI 可以回答的問題', 'moelog-ai-qna');
    ...
    if (defined('MOELOG_AIQNA_API_KEY') && MOELOG_AIQNA_API_KEY) {
        ...
        if (empty($input_key) || preg_match('/^\*+$/', $input_key)) {
            ...
            // 檢查是否已經是加密格式 (避免重複加密)
            if (function_exists('moelog_aiqna_is_encrypted') && 
                ...
                if (function_exists('moelog_aiqna_encrypt_api_key')) {
                    ...
                    if (!empty($encrypted_key)) {
                        ...
                        add_settings_error(
                            ...
                            __('⚠ API Key 加密失敗,已使用明文儲存。建議使用 wp-config.php 常數定義。', 'moelog-ai-qna'),
                            'warning'
                        );
                    }
                } else {
                    ...
                    if (defined('WP_DEBUG') && WP_DEBUG) {
                        error_log('[Moelog AIQnA] Encryption function not available, storing plaintext');
                    }
                }
            }
        }
    ...
    // 如果有其他欄位,在這裡繼續處理...
    // 例如: typing_speed, enable_analytics 等
            ...
            <ul style="margin:0;padding-left:20px;line-height:1.8;">
                ...
                <li>
                    <a href="<?php echo esc_url(admin_url('options-permalink.php')); ?>">
                        <?php esc_html_e('🔄 重新整理連結規則', 'moelog-ai-qna'); ?>
                    </a>
                </li>
            </ul>
        </div>
    ...
    private function render_usage_guide()
    {
        ...
        <ul style="list-style-type:circle;margin-left:20px;line-height:1.8;">
            <li>✅ <?php esc_html_e('加密 API Key 儲存', 'moelog-ai-qna'); ?></li>
            <li>✅ <?php esc_html_e('智慧預生成機制', 'moelog-ai-qna'); ?></li>
            <li>✅ <?php esc_html_e('靜態 HTML 快取機制', 'moelog-ai-qna'); ?></li>
...
public function show_notices()
{
    ...
    if (empty($api_key_set)) {
        ...
        echo '</strong> ';
        esc_html_e('尚未設定 API Key，請完成設定後才能使用 AI 功能。', 'moelog-ai-qna');
        echo '</p></div>';
    }
