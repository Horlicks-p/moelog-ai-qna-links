...
class Moelog_AIQnA_Renderer
{
    ...
    public function render_answer_page()
    {
        ...
        // 檢查靜態快取
        if (
            Moelog_AIQnA_Cache::exists($params["post_id"], $params["question"])
        ) {
            $this->serve_from_cache($params["post_id"], $params["question"]);
            return;
        }
    ...
    private function generate_and_render($params, $post)
    {
        ...
        // 建立 HTML
        $html = $this->build_html($params, $post, $answer);
     ...
     * @return string
     */
    private function build_html($params, $post, $answer)
    {
        // 標記為回答頁(供 GEO 模組使用)
        $GLOBALS["moe_aiqna_is_answer_page"] = true;

        // 準備模板變數
        $template_vars = $this->prepare_template_vars($params, $post, $answer);
     ...
     * @return array
     */
    private function prepare_template_vars($params, $post, $answer)
    {
        $settings = get_option(MOELOG_AIQNA_OPT_KEY, []);
        ...
        // Banner
        $vars["banner_url"] = apply_filters("moelog_aiqna_banner_url", "");
        $vars["banner_alt"] = apply_filters(
            "moelog_aiqna_banner_alt",
            $vars["site_name"]
        );

        // 免責聲明
        $vars["disclaimer"] = $this->get_disclaimer(
            $vars["site_name"],
            $settings
        );
        ...
        // 版本號
        $vars["version"] = MOELOG_AIQNA_VERSION;

        // 資源 URL
        $vars["plugin_url"] = MOELOG_AIQNA_URL;
        $vars["style_url"] =
            MOELOG_AIQNA_URL . "assets/style.css?ver=" . MOELOG_AIQNA_VERSION;
...
(function(){
  ...
  if(queue.length===0){
    ...
    private function set_security_headers()
    {
        ...
        $csp =
            "default-src 'self'; " .
            "script-src 'self' 'nonce-{$nonce}'; " . // ← 這裡加入 nonce
            "img-src 'self' data:; " .
            ...
            "base-uri 'self'; " .
            "form-action 'self'"; // 可選:若你確定是 Kaspersky 在本機開發造成阻擋,開發期臨時放行(正式環境不建議) // $csp .= " https://gc.kis.v2.scr.kaspersky-labs.com ws://gc.kis.v2.scr.kaspersky-labs.com"; // 若要加 report-uri
...
<div class="error-container">
  ...
  <a href="<?php echo esc_url(
     ...
     */
    public function render_for_pregeneration(
        $post_id,
        $question,
        $lang = "auto"
    ) {
        ...
        $params = [
            ...
            "question" => $question,
            "lang" =>
                $lang !== "auto" ? $lang : $this->detect_language($question),
        ];
        ...
        return $this->build_html($params, $post, $answer);
