...
class Moelog_AIQnA_AI_Client
{
    ...
    private function call_api($provider, $params)
    {
        ...
        if (
            ...
            // 若使用 OpenAI provider 卻指定 Gemini 模型，強制改回 GPT 系列
            ...
            if (defined("WP_DEBUG") && WP_DEBUG) {
                error_log(
                    "[Moelog AIQnA AI] 自動修正: OpenAI provider 不支援 Gemini 模型，改用 " .
                        $params["model"]
                );
            }
        }
        ...
        if (
            ...
            // 若使用 Gemini provider 卻指定 OpenAI 模型，強制改回 Gemini 系列
            ...
            if (defined("WP_DEBUG") && WP_DEBUG) {
                error_log(
                    "[Moelog AIQnA AI] 自動修正: Gemini provider 不支援 GPT 模型，改用 " .
                        $params["model"]
                );
            }
        }
