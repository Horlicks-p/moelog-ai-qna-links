...
class Moelog_AIQnA_AI_Client
{
    ...
    public function generate_answer($params)
    {
        ...
        // 生成快取鍵值
        $cache_key = $this->generate_cache_key($params, $settings);
        ...
        if ($cached !== false) {
            if (defined("WP_DEBUG") && WP_DEBUG) {
                error_log(
                    sprintf(
                        "[Moelog AIQnA AI] Cache hit for question: %s",
    ...
    private function call_openai($params)
    {
        ...
        if (is_wp_error($response)) {
            if (defined("WP_DEBUG") && WP_DEBUG) {
                error_log(
                    "[Moelog AIQnA AI] OpenAI HTTP Error: " .
    ...
    private function handle_openai_error($code, $json)
    {
        if (defined("WP_DEBUG") && WP_DEBUG) {
            error_log(
                sprintf(
                    "[Moelog AIQnA AI] OpenAI Error: HTTP %d, Response: %s",
                    $code,
                    wp_json_encode($json)
                )
            );
    ...
    private function call_gemini($params)
    {
        ...
        if (is_wp_error($response)) {
            if (defined("WP_DEBUG") && WP_DEBUG) {
                error_log(
                    "[Moelog AIQnA AI] Gemini HTTP Error: " .
                        $response->get_error_message()
                );
            }
    ...
    private function handle_gemini_error($code, $json)
    {
        if (defined("WP_DEBUG") && WP_DEBUG) {
            error_log(
                sprintf(
                    "[Moelog AIQnA AI] Gemini Error: HTTP %d, Response: %s",
                    $code,
                    wp_json_encode($json)
                )
            );
...
private function call_anthropic($params)
{
    ...
    $system_text = trim(($params['system_prompt'] ?? '') . "\n\n" . ($params['lang_hint'] ?? ''));
    ...
    if (is_wp_error($response)) {
        if (defined('WP_DEBUG') && WP_DEBUG) {
            error_log('[Moelog AIQnA AI] Anthropic HTTP Error: ' . $response->get_error_message());
        }
...
private function handle_anthropic_error($code, $json)
{
    if (defined('WP_DEBUG') && WP_DEBUG) {
        error_log(
            sprintf(
                '[Moelog AIQnA AI] Anthropic Error: HTTP %d, Response: %s',
                $code,
                wp_json_encode($json)
            )
        );
 ...
 private function call_api($provider, $params)
{
    ...
    if ($provider === 'openai' && stripos($params['model'], 'gemini') !== false) {
        ...
        if (defined('WP_DEBUG') && WP_DEBUG) {
            error_log('[Moelog AIQnA AI] 自動修正: OpenAI provider 不支持 Gemini 模型,改用 ' . $params['model']);
        }
    }
    ...
    if ($provider === 'gemini' && stripos($params['model'], 'gpt') !== false) {
        ...
        if (defined('WP_DEBUG') && WP_DEBUG) {
            error_log('[Moelog AIQnA AI] 自動修正: Gemini provider 不支持 GPT 模型,改用 ' . $params['model']);
        }
    }

    // ✅ 新增: Anthropic 的防呆檢查
    if ($provider === 'anthropic' && (stripos($params['model'], 'gpt') !== false || stripos($params['model'], 'gemini') !== false)) {
        ...
        if (defined('WP_DEBUG') && WP_DEBUG) {
            error_log('[Moelog AIQnA AI] 自動修正: Anthropic provider 不支持 GPT/Gemini 模型,改用 ' . $params['model']);
        }
    }

    // ✅ 新增: OpenAI 使用 Claude 模型的防呆
    if ($provider === 'openai' && stripos($params['model'], 'claude') !== false) {
        if (defined('MOELOG_AIQNA_DEFAULT_MODEL_OPENAI')) {
            $params['model'] = MOELOG_AIQNA_DEFAULT_MODEL_OPENAI;
        } else {
            $params['model'] = 'gpt-4o-mini';
        ...
        if (defined('WP_DEBUG') && WP_DEBUG) {
            error_log('[Moelog AIQnA AI] 自動修正: OpenAI provider 不支持 Claude 模型,改用 ' . $params['model']);
        }
    }

    // ✅ 新增: Gemini 使用 Claude 模型的防呆
    if ($provider === 'gemini' && stripos($params['model'], 'claude') !== false) {
        if (defined('MOELOG_AIQNA_DEFAULT_MODEL_GEMINI')) {
            $params['model'] = MOELOG_AIQNA_DEFAULT_MODEL_GEMINI;
        } else {
            $params['model'] = 'gemini-2.5-flash';
        ...
        if (defined('WP_DEBUG') && WP_DEBUG) {
            error_log('[Moelog AIQnA AI] 自動修正: Gemini provider 不支持 Claude 模型,改用 ' . $params['model']);
        }
    }
    // --------------------------------------------------------------------
        ...
        while ($retries <= self::MAX_RETRIES) {
            ...
            if ($retries < self::MAX_RETRIES) {
                ...
                if (defined("WP_DEBUG") && WP_DEBUG) {
                    error_log(
                        sprintf(
                            "[Moelog AIQnA AI] Retry %d/%d after %d seconds",
                            $retries,
                            self::MAX_RETRIES,
                            $delay
                        )
     ...
     * @return bool
     */
    private function is_error_message($text)
    {
        $error_keywords = [
            "失敗",
            "錯誤",
     ...
     * @return string
     */
    private function generate_cache_key($params, $settings)
    {
        $model =
            $settings["model"] ??
...
private function get_api_key()
{
    ...
    if (defined('MOELOG_AIQNA_API_KEY') && MOELOG_AIQNA_API_KEY) {
        ...
        if (defined('WP_DEBUG') && WP_DEBUG) {
            error_log('[Moelog AIQnA AI] Using API Key from wp-config.php constant');
        }
        return $key;
    }
    ...
    if ($stored_key === '') {
        $this->api_key_cache = '';
        if (defined('WP_DEBUG') && WP_DEBUG) {
            error_log('[Moelog AIQnA AI] No API Key found in database');
        }
        return '';
    }

    // 3) 若為加密/混淆格式 → 解密
    if (function_exists('moelog_aiqna_is_encrypted') && function_exists('moelog_aiqna_decrypt_api_key')) {
        if (moelog_aiqna_is_encrypted($stored_key)) {
            ...
            if ($decrypted !== '') {
                ...
                if (defined('WP_DEBUG') && WP_DEBUG) {
                    error_log('[Moelog AIQnA AI] API Key decrypted from database');
                }
                return $this->api_key_cache;
            } else {
                if (defined('WP_DEBUG') && WP_DEBUG) {
                    error_log('[Moelog AIQnA AI] Failed to decrypt stored API Key');
                }
                $this->api_key_cache = '';
                return '';
            }
    ...
    if (defined('WP_DEBUG') && WP_DEBUG) {
        error_log('[Moelog AIQnA AI] Using plaintext API Key from database');
    }
    return $this->api_key_cache;
}
...
{
    // 若帶入的是加密/混淆格式，先嘗試解密
    if (!empty($api_key) && function_exists('moelog_aiqna_is_encrypted') && function_exists('moelog_aiqna_decrypt_api_key')) {
