...
class Moelog_AIQnA_AI_Client
{
  ...
  private function call_anthropic($params)
  {
    ...
    if (is_wp_error($response)) {
      if (defined("WP_DEBUG") && WP_DEBUG) {
        error_log(
          "[Moelog AIQnA AI] Anthropic HTTP Error: " .
            $response->get_error_message(),
        );
      }
  ...
  private function call_api($provider, $params)
  {
    ...
    if (
      ...
      if (defined("WP_DEBUG") && WP_DEBUG) {
        error_log(
          "[Moelog AIQnA AI] 自動修正: OpenAI provider 不支持 Gemini 模型,改用 " .
            $params["model"],
        );
      }
    }
    ...
    if ($provider === "gemini" && stripos($params["model"], "gpt") !== false) {
      ...
      if (defined("WP_DEBUG") && WP_DEBUG) {
        error_log(
          "[Moelog AIQnA AI] 自動修正: Gemini provider 不支持 GPT 模型,改用 " .
            $params["model"],
        );
      }
    }
    ...
    if (
      ...
      if (defined("WP_DEBUG") && WP_DEBUG) {
        error_log(
          "[Moelog AIQnA AI] 自動修正: Anthropic provider 不支持 GPT/Gemini 模型,改用 " .
            $params["model"],
        );
      }
    }
    ...
    if (
      ...
      if (defined("WP_DEBUG") && WP_DEBUG) {
        error_log(
          "[Moelog AIQnA AI] 自動修正: OpenAI provider 不支持 Claude 模型,改用 " .
            $params["model"],
        );
      }
    }
    ...
    if (
      ...
      if (defined("WP_DEBUG") && WP_DEBUG) {
        error_log(
          "[Moelog AIQnA AI] 自動修正: Gemini provider 不支持 Claude 模型,改用 " .
            $params["model"],
        );
      }
    }
  ...
  private function get_api_key()
  {
    ...
    if (defined("MOELOG_AIQNA_API_KEY") && MOELOG_AIQNA_API_KEY) {
      ...
      if (defined("WP_DEBUG") && WP_DEBUG) {
        error_log(
          "[Moelog AIQnA AI] Using API Key from wp-config.php constant",
        );
      }
      return $key;
    }
    ...
    // 2) 從資料庫抓設定
    $settings = get_option(MOELOG_AIQNA_OPT_KEY, []);
    $stored_key = isset($settings["api_key"])
