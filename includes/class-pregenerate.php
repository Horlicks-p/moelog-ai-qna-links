...
class Moelog_AIQnA_Pregenerate
{
  ...
  public function pregenerate_answer($post_id, $question)
  {
    ...
    try {
      ...
      if (empty(trim($answer))) {
        throw new Exception("Empty answer returned from AI");
      }
      ...
      // ✅ 優化: 再次檢查 HTML 是否有效
      if (!$html || $this->is_error_response($html)) {
        throw new Exception(
          "Failed to render HTML or HTML contains error message",
        );
      }
   ...
   * ✅ 新增: 避免將錯誤寫入快取
   *
   * @param string $content 答案內容
   * @return bool
   */
  private function is_error_response($content)
  {
    if (empty($content)) {
      return true;
    }
  ...
  private function prepare_params($post_id, $question)
  {
    ...
    $lang = $index !== false && isset($langs[$index]) ? $langs[$index] : "auto";
  ...
  public function batch_pregenerate($post_id)
  {
    ...
    return [
      ...
      "skipped" => $skipped,
      "message" => sprintf(
        __("已排程 %d 個預生成任務 (跳過 %d 個已快取)", "moelog-ai-qna"),
        $scheduled,
        $skipped,
      ),
    ];
  ...
  public function batch_pregenerate_posts($post_ids)
  {
    ...
    return [
      ...
      "failed" => $failed,
      "message" => sprintf(
        __("已排程 %d 個預生成任務 (跳過 %d 個, 失敗 %d 篇)", "moelog-ai-qna"),
        $total_scheduled,
        $total_skipped,
        count($failed),
      ),
  ...
  public function cancel_all_tasks()
  {
    ...
    if (defined("WP_DEBUG") && WP_DEBUG) {
      moelog_aiqna_log(sprintf("Cancelled %d pregeneration tasks", $cancelled));
    }
        ...
        if ($hook === "moelog_aiqna_pregenerate") {
          foreach ($events as $key => $event) {
            ...
            $tasks[] = [
              ...
              "post_id" => $args[0] ?? 0,
              "question" => isset($args[1]) ? substr($args[1], 0, 50) : "",
            ];
          }
        }
      }
