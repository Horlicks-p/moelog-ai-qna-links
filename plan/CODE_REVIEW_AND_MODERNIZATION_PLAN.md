...
- 版本一致性自動檢查與單一發布版本來源。
- 在 plugin header 加入指向官方 GitHub repository 的 `Update URI`，明確宣告第三方更新來源，避免與 WordPress.org 同 slug 外掛發生更新來源衝突。
- GitHub updater 必須使用精確版本發布包 regex，並以 Plugin Update Checker 的 `REQUIRE_RELEASE_ASSETS` 模式強制要求 Release ZIP；若正式 asset 缺失或名稱不符，不得降級安裝包含開發檔案的 GitHub source archive。
- 加入 updater／release contract tests：驗證 updater library 缺失時不 fatal、只有精確命名的 Release ZIP 可被選取、遠端 package URL 指向 Release asset、ZIP 僅有單一正確頂層目錄、排除 tests／docs／plan／agent 設定，且 ZIP 內版本與 Git tag 一致；可選擇再由 CI 產生並核對 SHA-256 manifest，提高供應鏈可驗證性。
...
- 將 `CF-Connecting-IP` 的信任來源與一般 trusted proxy 分離：只有明確設定的 Cloudflare／CDN 網段才接受其專用標頭，避免非 Cloudflare proxy 未清理訪客自帶標頭時被偽造。
- 將 A3 transient 滑動視窗近似限流升級為可觀察、可選固定視窗且具原子更新能力的實作；跨節點部署需明確定義共享儲存。
...
2026-07-17 的 2.0.8 follow-up 完成剩餘 B 邊界：`CF-Connecting-IP` 改由獨立 Cloudflare／CDN trusted ranges 授權；feedback 限流改為固定視窗、non-autoload option 與條件 SQL 原子遞增，提供狀態 hook 並在拒絕時回傳 429／`Retry-After`。同版亦修復 `index.html` migration 與自訂 cache directory 卸載清理的一致性。跨節點共享儲存仍須由部署端提供一致的 WordPress database／object-cache policy，不在單站外掛內另建分散式服務。
...
- 將問題數上限集中為單一設定／常數，避免 Router、Feedback 與 Metabox 各自硬編碼 8。
...
- Plugin header 已宣告官方 `Update URI`；updater 只接受符合精確命名規則的 Release asset，缺少正式 ZIP 時不退回 GitHub source archive。
- updater／release contract tests 與 ZIP smoke test 可重現執行，並驗證單一頂層目錄、發布 allowlist 及 tag／內含版本一致性。
- `CF-Connecting-IP` 只有在直連來源命中獨立 Cloudflare／CDN trusted ranges 時才被接受；一般 trusted proxy 不得藉此標頭覆寫 XFF chain。
- Feedback limiter 使用可觀察的固定視窗與資料庫原子遞增；達限時回傳 HTTP 429／`Retry-After`，卸載時移除其 non-autoload counters。
- Cache migration 保留／修復 `index.html` 控制檔，lifecycle 清理使用解析後直接子目錄檢查並涵蓋所有 runtime 可用的自訂目錄名稱。
