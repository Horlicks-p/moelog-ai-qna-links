 ...
 * Description: 在每篇文章底部顯示作者預設的問題清單，點擊後開新分頁，由 AI 生成答案。支援 OpenAI，可自訂模型與提示。
 * Version: 1.1.0
...
class Moelog_AIQnA {
    ...
    public function register_settings() {
        ...
        add_settings_field('model', __('模型（OpenAI）', 'moelog-ai-qna'), function() {
            $o = get_option(self::OPT_KEY, []);
            $val = $o['model'] ?? 'gpt-4o-mini';
            printf(
                '<input type="text" style="width:320px" name="%s[model]" value="%s" placeholder="gpt-4o-mini">',
                esc_attr(self::OPT_KEY),
                esc_attr($val)
            );
        }, self::OPT_KEY, 'general');
    ...
    public function sanitize_settings($input) {
        ...
        $out['provider'] = in_array(($input['provider'] ?? 'openai'), ['openai'], true) ? $input['provider'] : 'openai';
    ...
    public function settings_page_html() {
        ...
        <div class="wrap">
            ...
            <ol>
                ...
                <li><?php esc_html_e('前台文章底部會顯示問題列表。點擊後開新分頁顯示 AI 答案。', 'moelog-ai-qna'); ?></li>
    ...
    public function questions_metabox_html($post) {
        ...
        <div id="moelog-aiqna-questions">
            ...
            <?php endforeach; ?>
            <p style="color:#666;"><?php esc_html_e('提示：每行一題，建議 3–8 題，每題最多 200 字。語言選擇「自動偵測」時，AI 會根據問題文字判斷語言。', 'moelog-ai-qna'); ?></p>
            <button type="button" onclick="moelogAddQuestionRow()"><?php esc_html_e('新增問題', 'moelog-ai-qna'); ?></button>
        </div>
        <script>
        function moelogAddQuestionRow() {
            const container = document.getElementById('moelog-aiqna-questions');
            ...
            row.innerHTML = `
                <textarea style="width:70%; min-height:60px; flex-grow:1; line-height:1.2em;" name="moelog_aiqna_questions[]" placeholder="<?php esc_attr_e('例：為何科技新創偏好使用「.io」？', 'moelog-ai-qna'); ?>"></textarea>
                <select name="moelog_aiqna_langs[]" style="margin-left:10px; min-width:120px;">
    ...
    private function get_questions_block($post_id) {
        ...
        foreach ($lines as $idx => $q) {
            ...
            // ts + HMAC 簽章，加入站內 secret
            $sig = hash_hmac('sha256', $post_id . '|' . $q . '|' . $ts, wp_salt('nonce') . '|' . $this->secret);
        ...
        return sprintf(
            ...
            esc_html__('他に知りたいことがありますか？ではAIに聞きましょう。間違ったら知りませんが…', 'moelog-ai-qna'),
            $items
        );
    }
    ...
    public function render_answer_page() {
        ...
        // 額外有效期：1 小時
        if (abs(time() - $ts) > HOUR_IN_SECONDS) {
            wp_die(__('連結已過期，請回原文重新點擊問題。', 'moelog-ai-qna'), __('錯誤', 'moelog-ai-qna'), ['response' => 403]);
        }

        // HMAC 檢查
        $expect = hash_hmac('sha256', $post_id . '|' . $question . '|' . $ts, wp_salt('nonce') . '|' . $this->secret);
        ...
        // ---- 快取優先 → 冷卻 → IP 限流 → 打 API ----
        $context_hash = substr(hash('sha256', $context), 0, 32); // 32 字元片段
        $cache_key = 'moe_aiqna_' . hash('sha256', $post_id . '|' . $question . '|' . $model . '|' . $context_hash);
            ...
            <div class="moe-container">
                ...
                <div class="moe-answer-wrap">
                    ...
                    (function() {
                        ...
                        // 將來源 DOM 攤平成「可逐字輸出的 text node 序列」，並同步建立目標結構
                        function prepareTyping(sourceParent, targetParent, queue) {
                            Array.from(sourceParent.childNodes).forEach(src => {
                                if (src.nodeType === Node.TEXT_NODE) {
                                    const tnode = document.createTextNode('');
                                    targetParent.appendChild(tnode);
                                    const text = src.textContent || '';
                                    if (text.length) queue.push({ node: tnode, text });
    ...
    private function call_ai($provider, $args) {
        ...
        // cache_key 一律由 render 端傳入；若缺失才極端 fallback
        $cache_key = $args['cache_key'] ?? ('moe_aiqna_' . hash('sha256', $post_id . '|' . $question . '|' . $model . '|' . substr(hash('sha256', $context), 0, 32)));
        ...
        switch ($provider) {
            ...
            default:
                ...
                if ($code === 401) {
                    return __('API Key 無效，請檢查設定。', 'moelog-ai-qna');
                }
                if ($code === 429) {
                    $msg = $json['error']['message'] ?? '';
                    if (strpos($msg, 'insufficient_quota') !== false) {
                        return __('API 信用已用盡或過期，請檢查 OpenAI 帳戶並添加信用。', 'moelog-ai-qna');
                    }
                    return __('請求過於頻繁，請稍候再試。', 'moelog-ai-qna');
                }
                return __('OpenAI 回傳異常，請稍後再試。', 'moelog-ai-qna');
        }
    }
    ...
    public static function uninstall() {
        ...
        // 清理 transient
        $like1 = $wpdb->esc_like('_transient_moe_aiqna_') . '%';
        $like2 = $wpdb->esc_like('_transient_timeout_moe_aiqna_') . '%';
        $wpdb->query(
            $wpdb->prepare(
