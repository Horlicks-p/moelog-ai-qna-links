...
class Moelog_AIQnA {
    ...
    public function register_settings() {
        ...
        add_settings_field('disclaimer_text', __('回答頁免責聲明', 'moelog-ai-qna'), function() {
            ...
            echo '<p class="description">'.esc_html__('支持 {site} 代表網站名稱,亦相容舊式 %s。可多行。', 'moelog-ai-qna').'</p>';
        }, self::OPT_KEY, 'general');
    }
    ...
    public function settings_page_html() {
        ...
        <div class="wrap">
            <h1><?php esc_html_e('Moelog AI Q&A', 'moelog-ai-qna'); ?> <span style="font-size:0.6em;color:#999;">v<?php echo esc_html(self::VERSION); ?></span></h1>
            ...
            <p><strong><?php esc_html_e('v1.5.0 更新:', 'moelog-ai-qna'); ?></strong></p>
            <ul>
                <li>✅ URL 極簡化: 從 226 字元縮短至 36 字元 (-84%)</li>
                <li>✅ 路徑簡化: /ai-answer/ → /qna/ (更簡潔)</li>
                <li>✅ 智慧縮寫: ICANN → ic, DNS → dns</li>
                <li>✅ Hash 優化: 8 字元 → 3 字元</li>
                <li>✅ 完全向下相容舊版 URL (1.3.2 和 1.4.3)</li>
            </ul>
        </div>
        <?php
    }
...
HTML;
    ...
    public function render_answer_page() {
        $req_uri = $_SERVER['REQUEST_URI'] ?? '';
        $is_pretty = (strpos($req_uri, '/' . self::PRETTY_BASE . '/') !== false) ||
                     (strpos($req_uri, '/ai-answer/') !== false);
        ...
        if (!$question) {
            ...
            if ($post_id && $question && $nonce && $ts && $sig) {
                ...
                if (!hash_equals($expect, $sig)) {
                    wp_die(__('簽章檢核失敗。feo', 'moelog-ai-qna'), [['response' => 403]]);
                }
            }
        }
    ...
    private function render_answer_html($post_id, $question, $answer) {
        ...
        $canonical = user_trailingslashit(home_url(self::PRETTY_BASE . '/' . $slug)); ?>
        <!doctype html>
